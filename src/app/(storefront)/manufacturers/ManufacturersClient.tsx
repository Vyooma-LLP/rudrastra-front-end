 ...
 */
export default function ManufacturersClient({ dbManufacturers = [] }: { dbManufacturers?: any[] }) {
  const [isApplied, setIsApplied] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  
  // Filter by search query on the client
  const manufacturers = dbManufacturers.filter(m => 
        ...
        {manufacturers.map((mfg) => (
          <Link key={mfg.id} href={mfg.link} className="bg-white border border-[var(--border)] p-8 flex flex-col h-full min-h-[340px] relative group hover:bg-[var(--primary)] transition-colors duration-fast">
            <div className="flex justify-between items-start mb-6">
              <span className="font-sans text-[14px] font-bold text-[var(--muted-foreground)] group-hover:text-white/85 transition-colors duration-fast truncate max-w-[120px]">{mfg.id.substring(0, 8).toUpperCase()}</span>
