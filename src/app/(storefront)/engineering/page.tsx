 ...
 * Compatibility Engine, BOM, and Architecture Hub.
 */
export default function EngineeringPage() {
  const tools = [
    ...
    },
    {
      ...
      link: "/engineering/architectures",
      comingSoon: true,
    }
  ];
    ...
    <div className="w-full flex flex-col min-h-screen bg-[#F5F5F5]">
      ...
      <section className="w-full bg-white border-b border-[var(--border)] px-6 lg:px-10 py-16">
        ...
        <p className="font-sans text-[16px] text-[var(--muted-foreground)] max-w-2xl mb-8">
          The technical core of Rudrastra. Validate system compatibility, resolve complex BOMs, and explore reference architectures for indigenous UAV development.
        </p>
      </section>
      ...
      <section className="w-full max-w-7xl mx-auto px-6 lg:px-10 py-16 grid grid-cols-1 lg:grid-cols-3 gap-6">
        {tools.map((tool, idx) => (
          <Link key={idx} href={tool.link} className={`bg-white border border-[var(--border)] p-8 flex flex-col h-full min-h-[320px] relative group transition-colors duration-fast ${tool.comingSoon ? "opacity-70 pointer-events-none grayscale" : "hover:bg-[#111111]"}`}>
            ...
            <h2 className="font-heading font-bold text-[28px] tracking-tight mb-4 group-hover:text-white transition-colors duration-fast">
              {tool.title}
              {tool.comingSoon && <span className="ml-3 text-[11px] font-sans uppercase tracking-widest text-black bg-[#EAEAEA] px-2 py-1 rounded-sm align-middle group-hover:text-[#111111]">Soon</span>}
            </h2>
            ...
            <div className="flex justify-between items-end mt-auto pt-4 border-t border-[var(--border)] group-hover:border-white/20 transition-colors duration-fast">
              <span className="font-sans font-bold text-[14px] flex items-center gap-2 group-hover:text-white transition-colors duration-fast">
                {tool.comingSoon ? "In Development" : "Launch Tool"}
                {!tool.comingSoon && (
      ...
      <section className="w-full bg-[#111111] text-white px-6 lg:px-10 py-24">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 border-b border-white/20 pb-6">
            ...
            <Link href="/engineering/architectures" className="hidden md:inline-flex px-6 py-3 border border-white text-[13px] font-sans font-semibold hover:bg-white hover:text-black transition-colors duration-fast mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {architectures.map((arch, idx) => (
              <Link key={idx} href={`/engineering/architectures/${arch.slug}`} className="bg-black/50 border border-white/10 p-6 hover:border-white/40 transition-colors duration-fast cursor-pointer group">
