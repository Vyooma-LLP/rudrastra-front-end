...
export default function CategoriesPage() {
  ...
  return (
    <div className="w-full flex flex-col min-h-screen bg-[#F5F5F5]">
      ...
      {/* HERO */}
      <section className="w-full bg-white border-b border-[var(--border)] px-6 lg:px-10 py-16 text-center flex flex-col items-center">
        <h1 className="font-heading font-extrabold text-[40px] md:text-[56px] leading-[1.05] tracking-tight mb-4 max-w-3xl">
          Browse by <span className="text-[var(--primary)]">Engineering</span> System
        </h1>
        <p className="font-sans text-[16px] text-[var(--muted-foreground)] max-w-xl">
      ...
      <section className="w-full max-w-7xl mx-auto px-6 lg:px-10 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories.map((cat) => (
          <Link key={cat.id} href={cat.link} className="bg-white border border-[var(--border)] p-8 flex flex-col h-[320px] relative group hover:bg-[var(--primary)] transition-colors duration-fast">
            ...
            <p className="font-sans text-[15px] text-[var(--muted-foreground)] leading-relaxed mb-6 flex-1 group-hover:text-white/90 transition-colors duration-fast">
